...
| Method | Path                       | Description |
| ------ | -------------------------- | ----------- |
| GET    | `/api/parts`               | List parts. Query params: `q` (name/notes/link substring), `sort` (`quantity_asc` default-friendly, `quantity_desc`, `name_asc`, `name_desc`, `recent`). Each row includes `model_count`, `model_names`, and `catalog_part_id` (when the part is trace-linked to a catalog part). |
...
| GET    | `/api/parts/:id`           | Part detail: `{part, models[]}` — all compatible models. `part` includes `catalog_part_id` (when set); then a `catalog` summary (`catalog_part_id`, `catalog_part_name`, `part_number`, `catalog_model_id`, `catalog_model_name`, `manufacturer`, `model_category`) is embedded so the detail page can show the link and offer unlink without a second round trip. |
| PUT    | `/api/parts/:id`           | Full replace update (same body as create). Never touches `catalog_part_id` — the catalog link can only be changed via `link-catalog` below. |
...
| POST   | `/api/parts/:id/link-catalog` | Trace-link this inventory part to a reference catalog part (sets `parts.catalog_part_id`). Body: `{"catalog_part_id": N}`. Re-linking re-points the target. 404 for unknown part or catalog part. Returns the refreshed part detail (with the `catalog` summary embedded). |
| DELETE | `/api/parts/:id/link-catalog` | Remove the part's catalog trace link. 404 when the part has no link. |
...
| GET    | `/api/catalog/parts`                         | Search catalog parts across all models. Query param `q`: case-insensitive substring on part name, part number, or notes; omitted lists the first 100 parts in name order. Each hit is the catalog part joined with `catalog_model_name`, `manufacturer`, and `model_category`. |
...
- `#/parts` — all parts, searchable, sortable (defaults to quantity low→high so out-of-stock floats to the top; 0 shows an "out" badge, and at-or-below the configured low-stock threshold a "low" badge). Parts trace-linked to a catalog part get a "catalog" chip. Rows are checkbox-selectable; selecting one or more opens a **bulk edit** panel that changes any of the part fields (set or clear) on every selected part at once, and links/unlinks a chosen model across the selection.
- `#/parts/:id` — part detail: quantity stepper, compatible models, link/unlink models, plus a **Catalog** section: linked parts show the catalog part/model/manufacturer with an unlink button; unlinked parts get a catalog-part search (name or part number) with per-result link buttons.
...
- **Catalog link & quantity semantics:** `models.catalog_model_id` is set only via `POST/DELETE /api/models/:id/link-catalog` (which validates category equality); `PUT /api/models/:id` deliberately does not touch it, so full-replace updates never wipe a catalog link. Inventory→catalog is deliberately a manual trace link, never automatic matching: `parts.catalog_part_id` is set only by the catalog "add to inventory" action or `POST/DELETE /api/parts/:id/link-catalog` (the part-detail Catalog section), and `PUT /api/parts/:id` can't wipe it. A hand-created part that happens to share a name or part number with a catalog part is not detected as a match — the user links it explicitly (the catalog search matches name, part number, and notes to make finding the right row easy). `owned_quantity` on `GET /api/catalog/models/:id` is `null` when no user model is linked (nothing to count against) and a live integer (0 when linked but not owned) otherwise; the optional `?model_id=` scopes it to one model. A part counts toward a catalog part's owned quantity only when it is trace-linked to that catalog part **and** linked to a user model that is linked to the catalog model. Add-to-inventory is idempotent per (catalog part, model): first call creates, later calls adjust the existing part's quantity (clamped at 0), so the same catalog part never yields duplicate inventory rows.
