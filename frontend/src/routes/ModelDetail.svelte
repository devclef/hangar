...
{:else if detail}
  <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
    <a href="#/models" class="text-sm text-stone-500 hover:text-stone-800">← All models</a>
  ...
  <div class="card p-5">
    <div class="flex flex-wrap items-center gap-3">
      <h1 class="text-2xl font-bold text-zinc-900">{detail.model.name}</h1>
    ...
    <dl class="mt-4 grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:grid-cols-3">
      <div>
        ...
        <dd class="text-stone-700">{detail.model.manufacturer ?? '—'}</dd>
      </div>
      <div>
        <dt class="label">Acquired</dt>
        <dd class="text-stone-700">{formatDate(detail.model.date_acquired)}</dd>
      </div>
      <div>
    ...
    {#if detail.model.notes}
      <p class="mt-3 whitespace-pre-wrap text-sm text-stone-600">{detail.model.notes}</p>
    {/if}
  ...
  <div class="card mt-6">
    <div class="flex items-center justify-between border-b border-stone-200 px-4 py-3">
      <h2 class="text-sm font-semibold uppercase tracking-wide text-stone-600">
        Parts in stock <span class="font-normal normal-case text-stone-400">({detail.parts.length})</span>
      </h2>
    </div>
          ...
          <tbody class="divide-y divide-stone-100">
            {#each detail.parts as p (p.id)}
              <tr class="transition-colors hover:bg-stone-50">
                ...
                <td class="td max-w-xs truncate text-stone-500" title={p.notes ?? ''}>{p.notes ?? '—'}</td>
